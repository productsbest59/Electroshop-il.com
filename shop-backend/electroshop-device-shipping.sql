begin;
CREATE OR REPLACE FUNCTION public.create_electroshop_bit_order(p_customer jsonb, p_items jsonb, p_request_id uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
begin
 ...
 for v_item in select value from jsonb_array_elements(p_items) loop
   ...
   if v_method='shipping' and (v_product.category='smartphones' or v_product.sku like 'PELEPHONE-%') then v_shipping:=50; end if;
 ...
 update public.electroshop_orders set subtotal=v_total,shipping_amount=v_shipping,total=v_total+v_shipping,payment_provider='bit_manual',bit_request_id=p_request_id where id=v_id;
