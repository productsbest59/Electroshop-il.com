begin;
alter table public.electroshop_settings add column if not exists bit_enabled boolean not null default false;
alter table public.electroshop_orders add column if not exists bit_request_id uuid;
create unique index if not exists electroshop_bit_request_unique on public.electroshop_orders(bit_request_id) where bit_request_id is not null;
create or replace function public.create_electroshop_bit_order(p_customer jsonb,p_items jsonb,p_request_id uuid) returns jsonb language plpgsql security definer set search_path=public,pg_temp as $$
declare
...
begin
 if not coalesce((select bit_enabled from public.electroshop_settings where id),false) then raise exception 'התשלום בביט אינו זמין כרגע'; end if;
 if p_request_id is null then raise exception 'מזהה בקשה חסר'; end if;
 perform pg_advisory_xact_lock(hashtextextended(p_request_id::text,0));
 select * into v_existing from public.electroshop_orders where bit_request_id=p_request_id;
 if found then return jsonb_build_object('id',v_existing.id,'order_number',v_existing.order_number,'total_ils',v_existing.total,'payment_status',v_existing.payment_status); end if;
 ...
 update public.electroshop_orders set subtotal=v_total,total=v_total+v_shipping,payment_provider='bit_manual',bit_request_id=p_request_id where id=v_id;
...
revoke all on function public.create_electroshop_bit_order(jsonb,jsonb,uuid) from public;
grant execute on function public.create_electroshop_bit_order(jsonb,jsonb,uuid) to anon,authenticated;
update public.electroshop_settings set bit_enabled=true,shipping_amount=0 where id;
commit;
