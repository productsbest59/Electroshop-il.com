...
function render(){
 ...
 if(page){const c=categories.find(c=>c.slug===page.dataset.category||(c.aliases||[]).includes(page.dataset.category));if(c){if(page.querySelector('h1'))page.querySelector('h1').textContent=c.slug==='smartphones'?(english()?'Devices':'מכשירים'):c['name_'+language];if(page.querySelector('.shop-intro p'))page.querySelector('.shop-intro p').textContent=c['description_'+language];if(page.querySelector('.eyebrow'))page.querySelector('.eyebrow').textContent='ELECTROSHOP / '+c['name_'+language];if(page.querySelector('.breadcrumb'))page.querySelector('.breadcrumb').textContent=(english()?'Our store / ':'החנות שלנו / ')+c['name_'+language];}}
}
