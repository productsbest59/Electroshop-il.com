...
function render(){
 ...
 if(page){const c=categories.find(c=>c.slug===page.dataset.category);if(c){page.querySelector('h1').textContent=c['name_'+language];page.querySelector('.shop-intro p').textContent=c['description_'+language];page.querySelector('.eyebrow').textContent='ELECTROSHOP / '+c['name_'+language];if(page.querySelector('.breadcrumb'))page.querySelector('.breadcrumb').textContent=(english()?'Our store / ':'החנות שלנו / ')+c['name_'+language];}}
}
