...
function render(){
 ...
 if(page){const c=categories.find(c=>c.slug===page.dataset.category||(c.aliases||[]).includes(page.dataset.category));if(c){if([c.slug,...(c.aliases||[])].includes('speakers')){const intro=page.querySelector('.shop-intro');if(intro&&!intro.querySelector('[data-speakers-banner]')){const banner=document.createElement('img');banner.dataset.speakersBanner='';banner.className='shop-category-banner';banner.src='images/speakers-banner.png';banner.alt='רמקולים ובידוריות';banner.width=2048;banner.height=768;banner.fetchPriority='high';banner.style.cssText='display:block;width:100%;height:auto;object-fit:contain;max-width:100%;margin:auto';intro.replaceChildren(banner)}}if(page.querySelector('h1'))page.querySelector('h1').textContent=c.slug==='smartphones'?(english()?'Devices':'מכשירים'):c['name_'+language];if(page.querySelector('.shop-intro p'))page.querySelector('.shop-intro p').textContent=c['description_'+language];if(page.querySelector('.eyebrow'))page.querySelector('.eyebrow').textContent='ELECTROSHOP / '+c['name_'+language];if(page.querySelector('.breadcrumb'))page.querySelector('.breadcrumb').textContent=(english()?'Our store / ':'החנות שלנו / ')+c['name_'+language];}}
}
...
function addForm(list,c){
 ...
 form.onsubmit=async e=>{
   ...
   try{
     ...
     Object.assign(c,body);refreshSummary();localStorage.setItem('electroshop_products_updated',crypto.randomUUID());adminSession=session;await load();Object.assign(c,categories.find(row=>row.slug===slug));document.dispatchEvent(new Event('electroshop-categories-updated'));message.textContent='הקטגוריה נשמרה. שיוך המוצרים והקישורים הקודמים נשמרו.';
